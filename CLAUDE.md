...
Точное поведение `sale_price` при отсутствии скидки пока не придумывать — определить по требованиям курса.
...
Текущий незакрытый пункт:

- после появления `CLAUDE.md` в GitHub проверить `/mcp`;
- целевое состояние GitHub MCP: `connected`.
